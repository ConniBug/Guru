...
public abstract class CodewarsCommand extends Command{
	...
	public Optional<MessageCreateAction> sendUpdateMessage(String id, MessageReceivedEvent event) {
		if(Guru.getInstance().getUsersHandler().needsUpdating(id)) {
			...
			return Optional.of(event.getMessage().replyEmbeds(embedBuilder.build()));
		}
